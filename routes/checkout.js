  ...
  Discount.getDiscountByCode(discountCode, function(e, discount) {
    if (e) {
      ...
      if (discount) {
        ...
        totalDiscount = parseFloat(totalDiscount.toFixed(2)).toFixed(2) //BEN
        let totalPrice = cart.totalPrice - totalDiscount;
        totalPrice = parseFloat(totalPrice.toFixed(2)).toFixed(2) //BEN
...
router.post('/checkout-process', function(req, res) {
  ...
  var create_payment_json = {
      ...
      "redirect_urls": {
          "return_url": "http://134.117.216.44:3000/checkout/checkout-success",
          "cancel_url": "http://134.117.216.44:3000/checkout/checkout-cancel"
      },
      "transactions": [{
          "item_list": {
              "items": [{
...
router.get('/checkout-success', ensureAuthenticated, function(req, res) {
    ...
    paypal.payment.execute(paym, execute_payment_json, function (error, payment) {
        if (error) {
  ...
  // empty cart
  req.session.cart = req.session.cart.constructor();
